...
with data in a plain folder. PGLite allows exactly **one** connection, so the entire
architecture follows from one rule: **`memloom serve` is the single owner of the store, and
every other surface is a client of the daemon.**
...
function (`memloom_fuse`) is `LANGUAGE sql`, not plpgsql, precisely so it runs identically on
...
- **The edge table has no foreign keys.** `memory_edges` links memories, entities, *and*
  context chunks in one table — different node tables, so FKs are impossible by design. The
