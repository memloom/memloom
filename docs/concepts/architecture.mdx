...
memloom's default store is PGLite: real Postgres compiled to WASM, embedded in the daemon
process, with data in a plain folder. PGLite allows exactly **one** connection, so
**`memloom serve` is the single owner of the store, and every other surface is a client of
the daemon.** (The same daemon runs on an external Postgres via `MEMLOOM_PG_URL`; the
single-owner API model stays, only the storage moves out of the process.)
...
(which also serves the viewer bundle at `/`), and, on the embedded tier, the raw Postgres
wire protocol on `127.0.0.1:54329` for database tools (with `MEMLOOM_PG_URL` set there is
no bridge; point your tools at the server). Any CLI command auto-starts the daemon if it
isn't running; `memloom stop` shuts it down gracefully and releases the data-dir lock.
...
| `@memloom/mcp` | The [MCP tools](/mcp/tools) for agents: save, recall, read_passage, history, conflicts, and schema management. |
...
- **`PgAdapter`**: a real Postgres server over the wire (Docker, Supabase, any managed
  Postgres with pgvector), pooled. Selected with `MEMLOOM_PG_URL` in `config.env`; see
