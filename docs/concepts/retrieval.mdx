...
It anchors on at most **10** entities per query, which is why duplicate spellings of one
thing cost real recall quality: each one spends a slot. The arm also anchors on the spellings
that [entity resolution](/concepts/entity-resolution) has folded away, mapping each back to
the entity it now belongs to, so a query using an old spelling reaches everything the
surviving entity is attached to. A folded spelling and its canonical are counted once between
them, not twice.
