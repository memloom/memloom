...
Vector search alone misses exact identifiers — `ECONNREFUSED`, `pg_hba.conf`, a ticket
number — because embeddings blur precise strings into approximate meaning. Keyword search
alone misses paraphrases. memloom runs both (plus an entity arm) inside the database, in a
...
known entity (query ↔ entity cosine ≥ 0.45). A vague query anchors to nothing and the arm
contributes nothing, so it never drags in unrelated results. The keyword arm abstains
naturally the same way — no lexical match, no votes.

## Reciprocal-rank fusion

Each arm produces a ranking; fusion combines *ranks*, not scores (cosine distances and FTS
...
Default weights: vector **1.0**, keyword **2.0**, entity **1.0** — keyword is up-weighted
because it abstains when it has nothing, so its votes are cheap and precise (eval-tuned).
A result that two arms agree on beats a result that tops one arm alone.

Each recall result carries both numbers:

- `rrfScore` — the fused rank score. **This is the order results arrive in.**
- `similarity` — the raw cosine signal alone, useful as a "how semantically close" gauge but
  not what the ranking used.
...
- **Chunks embed their breadcrumb.** A markdown chunk is stored as
  ...
  the vector and the keyword arm see the context, not just the paragraph. (This approximates
  Anthropic's contextual-retrieval result without a second LLM pass.)
...
<Note>
In offline mode (no API key) the vector arm runs on deterministic hash embeddings — texts
sharing words score as similar, which is a real but crude signal. The keyword arm works at
full quality offline; the entity arm needs indexing, which needs the LLM. For production
recall quality, configure a key. See [Configuration](/guides/configuration).
</Note>
