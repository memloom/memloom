...
`~/.claude/projects`. `memloom import claude-code` and `memloom connect claude-code` both
...
This is what makes `import claude-code` cheap to re-run: an unchanged session costs one
...
`importClaudeCode` runs against just that path, skipping discovery. It's still bounded: a
