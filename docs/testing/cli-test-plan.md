...
- [x] Stop any daemon: `memloom stop` (a "no daemon" style answer is fine).
- [x] Park your real store: rename `%USERPROFILE%\.memloom` to `.memloom-backup`.
      EXPECTED: the folder moves with no locked-file complaint. If it complains, a
      daemon or DB client still holds it.
- [x] `node --version` is >= 22 (the engines requirement).
...
- [x] Create `notes.md` with a couple of headed sections; `memloom context add notes.md`
...
- [x] Put a real `OPENROUTER_API_KEY` in `config.env`, then `memloom stop` and
      `memloom serve`.
...
- [x] Delete `~/.memloom/data` (keep `config.env`), `memloom serve` again.
...
- [x] `memloom save "the deploy window is friday afternoon"`
      EXPECTED: `added`.
- [x] Save a reworded restatement: `memloom save "deploys happen friday after lunch"`
      EXPECTED: `versioned` (new version of the same belief) or `merged`; either is a
      pass, `added` is a fail (dedup missed).
- [x] Save a contradiction: `memloom save "the deploy window moved to monday morning"`
...
- [x] `memloom context add <folder with a few .md/.txt/.pdf files>`
      EXPECTED: per-file outcomes, folder recursion works, unsupported files skipped.
- [x] Wait ~5 seconds after the add, then `memloom schema`.
...
- [x] `memloom auto-index` then `memloom auto-index off` then `on`.
...
- [x] Connect TablePlus/psql to `postgresql://postgres@127.0.0.1:54329/postgres`,
...
- [x] `memloom stop`; copy `~/.memloom/data` somewhere else; delete the original;
...
- [x] Restore your real store: stop the daemon, delete the test `.memloom`, rename
      `.memloom-backup` back.
- [x] File every deviation you wrote down. Anything in phases 1 to 4 is a launch
