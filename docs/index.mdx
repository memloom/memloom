...
This page gets you running. For how the engine actually works, start with
...
lines and numbered points (`2. DEFINICJA 2. …`) — one point per chunk, so a chunk is exactly
...
`from notes.pdf › GRANICA FUNKCJI > 2. DEFINICJA 2. (p. 1)`.

PDF text is rebuilt from glyph geometry, not stream order — equation-heavy PDFs (Word/LaTeX
math) come out in reading order, and 2-up print layouts (the same content twice, side by
...
chunks — and when memloom's extraction pipeline improves, the pipeline version bumps and
