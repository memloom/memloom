...
| Postgres wire | `postgresql://postgres@127.0.0.1:54329/postgres`: for Drizzle Studio, psql, TablePlus (default port, `MEMLOOM_PG_PORT` moves it; embedded tier only) |
| Data | `~/.memloom/data` |
...
(Drizzle Studio, psql, an IDE database panel) is connected on the wire port, it holds an exclusive
