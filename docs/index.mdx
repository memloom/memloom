...
lines and numbered points (`2. DEFINICJA 2. …`) — so a chunk never starts mid-definition.
Each chunk carries its breadcrumb and PDFs keep page numbers, so results from files always
say where they came from: `from notes.pdf › GRANICA FUNKCJI > 2. DEFINICJA 2. (p. 1)`.
