...
Documents are **mirrors** of files, not beliefs: no dedup, no conflicts, no versions.
...
PDF text is rebuilt from **glyph geometry**, not stream order: equation-heavy PDFs (Word or
...
**Markdown** splits at ATX headings (headings inside code fences are content, not structure),
...
context, not just the paragraph.

**Outline** handles documents without markdown structure — lecture notes, contracts,
exercise sheets. It recognizes ALL-CAPS title lines (Unicode-aware, so `GRANICA NIEWŁAŚCIWA
FUNKCJI` qualifies) and numbered points (`2. DEFINICJA 2. …`) as boundaries. One point = one
chunk, so a chunk is exactly one definition, theorem, or clause, with a breadcrumb like
`GRANICA FUNKCJI > 2. DEFINICJA 2.`. Text with no such structure degrades gracefully to plain
size-based splitting.

Within a section, chunks target ~1,600 characters (≈400 tokens) with a 2,048 hard cap and
~200 characters of overlap carried across boundaries — research-consensus defaults, sized in
characters on purpose (a tokenizer dependency buys nothing when the embedding model has ~20×
context headroom).

## Adding a format

The registry is open: one object, no fork.
