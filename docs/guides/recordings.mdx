...
<Steps>
  ...
  <Step title="Then the whole file">
    Still nothing, and the file is not silent? The entire recording goes to the recognizer, which
    is a better judge than the detector.
  </Step>
  <Step title="Refused only when there is nothing to keep">
    A silent file, or one the recognizer turns into no words at all, is refused with the reason.
    Storing a document whose only chunk is "Recorded on ..." would be worse than saying so.
  </Step>
</Steps>

<Warning>
  Speech as quiet as the music or noise over it usually cannot be recovered. The recognizer is a
  speech model, not a source separator, and no threshold changes that. For a recording to be
  usable the voice has to be the loudest thing in it.
</Warning>
