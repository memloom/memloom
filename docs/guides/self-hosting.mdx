...
The embedded tier is the right default: zero setup, zero services, backup = copy a folder.
...
The store is the folder. Stop the daemon first so the data directory is quiescent:
...
`pg` is an optional dependency — `pnpm add pg` in the project that runs this. `init()`
...
network — unless you publish the ports yourself, which is your call to make.
