...
bridge at all (inspect the server directly with any Postgres client).
...
`127.0.0.1:54329` by default (`memloom serve` prints the live port, which `MEMLOOM_PG_PORT`
can move), so with the daemon running you can pull data out with standard tools and load it
into the Docker database:
