...
embedded tier) the Postgres wire on 54329 (`MEMLOOM_PG_PORT` moves it; if the port will not
bind the daemon says so and serves on without the wire). With `MEMLOOM_PG_URL` set, the daemon
connects to your Postgres instead and skips the wire bridge. The daemon is the single owner of the
